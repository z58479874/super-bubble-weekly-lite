(function () {
  ...
  function render(topic, previous, record, { editable = false, preface = "" } = {}) {
    return `<main class="content dy-topic-page">${preface}<section class="dy-topic-hero"><div><span>抖音经营详情</span><h1>从成交表现到到店价值</h1><p>周期：${esc(topic?.period?.startDate || "未统计")} 至 ${esc(topic?.period?.endDate || "未统计")} · 数据截至 ${esc(topic?.period?.dataAsOf || "未统计")}</p></div>${statusTag(topic?.status)}</section>${summary(topic, previous)}${liveSection(topic)}${productSection(topic)}${valueChain(topic)}${qualitySection(topic)}${notesSection(record, editable)}</main>`;
  }
