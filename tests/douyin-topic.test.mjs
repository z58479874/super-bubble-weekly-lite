...
const input = {
  ...
  orders: [{ "订单编号": "800000000000000001", "下单时间": "2026-09-01 09:00:00", "支付时间": "2026-09-01 09:00:00", "订单状态": "待使用", "商品ID": "100000000000000001", "商品名称": "亲子票", "购买数量": 2, "用户实付": 200, "订单实收": 200, "预计收入": 180, "退款金额总计": 0, "成交渠道": "直播", "带货角色": "商家" }],
  redemptions: [
  ...
  aftersales: [{ "售后编号": "AF-1", "订单编号": "800000000000000001", "售后状态": "已退款", "售后完成时间": "2026-09-03 12:00:00", "退款金额": 20, "退款券数": 1 }]
};
...
test("退款按售后完成时间单独统计，不与订单退款重复相加", () => {
  const topic = buildDouyinTopics(input).topics[week.id];
  assert.equal(topic.aftersales.amount, 20);
  assert.equal(topic.aftersales.periodBasis, "售后完成时间");
  assert.equal(topic.orders.refund, 0);
});
