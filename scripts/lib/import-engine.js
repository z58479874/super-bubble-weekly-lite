(function(root){
  ...
  const TYPES={
    ...
    sales:{label:"销售办卡数据",required:["日期","姓名","合计办卡数","299","599","999"]},
  ...
  function recognize(fileName,book){
    ...
    for(const sheet of book.SheetNames){
      ...
      for(let i=0;i<rows.length;i++){
        ...
        else if(set.has("合计办卡数")&&(set.has("姓名")||set.has("员工")))type="sales";
  ...
  function makePeriods(operationRows){
    ...
    return Object.entries(groups).sort(([a],[b])=>a.localeCompare(b)).slice(-3).map(([weekStart,days])=>{
      const start=days[0],end=days.at(-1),naturalEnd=plusDays(weekStart,6),partial=start!==weekStart||end!==naturalEnd;
  ...
  function opsAdapter(item){
    const rows=rowsOf(item.book,item.rec.sheet),header=item.rec.headerIndex,main=rows[header]||[],sub=rows[header+1]||[];
    const headers=main.map((value,i)=>text(value)||(i?text(main[i-1]):"")).map((value,i)=>text(sub[i])&&text(sub[i])!==value?`${value}/${text(sub[i])}`:value);
    const records=rows.slice(header+2).filter(row=>row.some(value=>value!==null&&value!=="")).map(row=>Object.fromEntries(headers.map((key,i)=>[key||`__${i}`,row[i]??null])));
    const normalized=records.map(row=>({...row,__date:sheetDate(row["日期"],item.rec.sheet)})).filter(row=>row.__date&&n(row["总业绩"])!==null&&text(row["天气"]));
    item.info=sourceInfo(item.file,item.book,item.rec,normalized,normalized.map(x=>x.__date));
    return normalized;
  }
  ...
  function salesAdapter(rows,periods){
    ...
    periods.forEach(p=>{
      ...
      selected.forEach(row=>{
        ...
        const family=n(row["家庭数"]),amount=n(row["办卡总金额"]);if(family===null)x.receptionKnown=false;else x.reception+=family;if(amount===null)x.amountKnown=false;else x.amount+=amount;
        x.enrolled+=n(row["合计办卡数"])||0;x.c299+=n(row["299"])||0;x.c599+=n(row["599"])||0;x.c999+=n(row["999"])||0;x.legacy+=add(n(row["500"]),n(row["800"]));
      });
  ...
  function aftersalesAdapter(rows,period){
    const seen=new Set();let applications=0,amount=0,vouchers=0;rows.filter(row=>inPeriod(row.__date,period)).forEach(row=>{const id=text(row["售后编号"]);if(!id||seen.has(id))return;seen.add(id);applications++;amount+=n(row["退款金额"])||0;vouchers+=n(row["退款券数"])||0});return {applications,amount,vouchers};
  }
  ...
  async function parseFiles(files){
    ...
    const result={schemaVersion:"lite-real-v2-net-revenue",generatedAt:new Date().toISOString(),weeks,reception,receptionByWeek,douyinFacts,departments:DEPARTMENTS,quality:{errors,warnings:[...sales.warnings,...importWarnings],imports:items.map(x=>x.info),reconciliation:reconcile(weeks,douyinFacts),sourcePolicy:{operations:"运营报表（总业绩与旧线上金额仅作历史对账）",sales:"销售办卡数据/Sheet1",live:"新版直播数据",product:"新版商品数据（商品核销对账）",orders:"订单成交明细",redemptions:"核销明细/有效记录订单实收（经营权威）",aftersales:"售后申请明细（增强）",video:"短视频数据（增强）",revenuePolicy:"退款后净经营营业额",douyinPolicy:"GMV、支付、核销订单实收、预计收入、渠道费用、实际到账分别保存"}}};
    return result;
  }
